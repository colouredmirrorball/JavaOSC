// SPDX-FileCopyrightText: 2001-2014 C. Ramakrishnan / Illposed Software
...
public class OscUI extends JPanel {
	...
	protected void addOscServerAddressPanel() {
		...
		// variable setAddressButton holds an instance of JButton with
	...
	public void addGlobalControlPanel()
	{
		...
		globalOnButton.addActionListener(evt ->
		{
		...
		globalOffButton.addActionListener(evt ->
		{
	...
	// create method for adding the buttons and synths of the
	...
	public void addFirstSynthPanel() {
		...
		// set the initial value of the slider to 400
		slider.setEnabled(false);

		textBox = new JTextField(String.valueOf(0), 8);
		textBox.setEnabled(false);

		firstSynthButtonOn.addActionListener(evt ->
		{
		...
		firstSynthButtonOff.addActionListener(evt ->
		{
		...
		textBox.addActionListener(evt ->
		{
			final JTextField field = (JTextField) evt.getSource();
			float            freq  = Float.parseFloat(field.getText());
	...
	///********************
	// create method for adding the Second Synth Panel
	protected void addSecondSynthPanel() {
		...
		textBox2 = new JTextField(String.valueOf(0), 8);
		textBox2.setEnabled(false);

		secondSynthButtonOn.addActionListener(evt ->
		{
		...
		secondSynthButtonOff.addActionListener(evt ->
		{
		...
		// invoked; send the argument for freq and node
		textBox2.addActionListener(evt ->
		{
			final JTextField field = (JTextField) evt.getSource();
			float            freq  = Float.parseFloat(field.getText());
			if (freq > (FREQ_MIN + FREQ_MAX))
	...
	protected void addThirdSynthPanel() {
		...
		textBox3 = new JTextField(String.valueOf(0), 8);
		textBox3.setEnabled(false);

		thirdSynthButtonOn.addActionListener(evt ->
		{
			// when the action occurs the doSendOn method is invoked
		...
		thirdSynthButtonOff.addActionListener(evt ->
		{
			// when the action occurs the doSendOff method is invoked
		...
		// when the value in the textbox is changed, doSendSlider method is
		// invoked; send the argument for freq and node
		textBox3.addActionListener(evt ->
		{
			final JTextField field = (JTextField) evt.getSource();
			float            freq  = Float.parseFloat(field.getText());
			if (freq > (FREQ_MIN + FREQ_MAX))
	...
	public void doSendGlobalOff(final int node1, final int node2, final int node3) {
		...
		final int  delayTime = Integer.parseInt(textBox4.getText());
		time = time + delayTime;
